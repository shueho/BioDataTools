...
sort_choice <- 3    # 排序方式：1=按最高丰度分类从小到大, 2=从大到小, 3=按分组文件, 4=层次聚类
stip_p <- "top"     #分组标签位置，还可以设置在下方bottom

strip_color <- "#CCCCCC"  #分组标签背景色
...
p_sample <- ggplot(df_long_sample, aes(x = Sample, y = RelAbundance, fill = Taxon)) +
  ...
  facet_wrap(~Group, scales = "free_x", space = "free_x",nrow = 1, strip.position = stip_p) +
  theme_bw() +
  theme(
...
ggsave("06_sample_barplot.pdf", p_sample, width = 14, height = 6)
