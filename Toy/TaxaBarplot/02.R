...
taxa_choice <- 6    # 分类等级选择：0=Domain(域), 1=Kingdom(界), 2=Phylum(门), 3=Class(纲), 4=Order(目), 5=Family(科), 6=Genus(属), 7=Species(种)
sort_choice <- 1    # 排序方式：1=按最高丰度分类从小到大, 2=从大到小, 3=按分组, 4=层次聚类
border_color <- "grey50"   # 柱状图边框颜色

alluvial_alpha <- 0.4      # 间隙流线透明度（0~1）
curve_choice <- 1          # 流线曲线类型：1=linear(直线), 2=xspline(平滑曲线)
# ----------------------------------
...
if (taxa_choice < 0 | taxa_choice > 7) stop("taxa_choice 必须为 0-7")
...
curve_type <- switch(as.character(curve_choice),
                     "1" = "linear",
                     "2" = "xspline",
                     stop("curve_choice 必须为 1 或 2"))
...
normal_colors <- if (n_normal <= length(set2_colors)) {
  ...
  rep(set2_colors, length.out = n_normal)
}
...
p_sample <- ggplot(df_long_sample,
                   aes(x = Sample, stratum = Taxon, alluvium = Taxon,
                       y = RelAbundance, fill = Taxon)) +
  geom_alluvium(aes(fill = Taxon), alpha = alluvial_alpha,
                width = 0.8, curve_type = curve_type) +
  geom_stratum(width = 0.8, color = border_color, linewidth = 0.2) +   
...
p_group <- ggplot(df_long_group,
                  aes(x = Group, stratum = Taxon, alluvium = Taxon,
                      y = RelAbundance, fill = Taxon)) +
  geom_alluvium(aes(fill = Taxon), alpha = alluvial_alpha,
                width = 0.6, curve_type = curve_type) +
  geom_stratum(width = 0.6, color = border_color, linewidth = 0.2) +  
  scale_y_continuous(labels = scales::percent, expand = expansion(mult = c(0, 0.05))) +
