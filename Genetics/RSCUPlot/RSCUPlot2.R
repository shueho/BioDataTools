...
plot_rscu = function(rscu_d, out_path,h1=0.6,h2=12,yLim=6.2){
...
main_fun = function(pcg,codonTable,gene="Total"){
  ...
  aaa_counter <- list()
  for (i in 1:nrow(df)) {
    if (!df$AAA[i] %in% names(aaa_counter)) {
      # 首次出现，初始化计数器为1
      aaa_counter[[df$AAA[i]]] <- 8
      df$flag[i] <- aaa_counter[[df$AAA[i]]]  # 初始flag为8
    } else {
      aaa_counter[[df$AAA[i]]] = aaa_counter[[df$AAA[i]]] - 2
      df$flag[i] <- aaa_counter[[df$AAA[i]]]
    }
  }
  ...
  df <- df %>%
    ...
    mutate(
      row_num = row_number(),
      AAA = AAA #ifelse(n() > 4 & row_num <= 4, paste0(AAA, "1"),
